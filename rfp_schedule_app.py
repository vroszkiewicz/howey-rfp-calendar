...
def next_valid_business_day(date, holiday_list):
    adjusted = False
    while date.weekday() >= 5 or date in holiday_list:  # Skips Saturday (5) and Sunday (6)
...
if rfp_posted_date:
    ...
    # Determine next council meeting (2nd or 4th Monday)
    council_meeting = get_next_2nd_or_4th_monday(rfp_posted_date)
    if council_meeting:
        schedule["Presented to Town Council"] = council_meeting
        adjustments["Presented to Town Council"] = False
